# CC PROMPT — v2.31.5 — feat(security): encryption key rotation CLI

## What this does
Adds the missing ability to rotate `SETTINGS_ENCRYPTION_KEY`. Today, if the key is
ever exposed, the only remedy is to re-enter every credential by hand. This
prompt adds `scripts/rotate_encryption_key.py`: decrypts every `enc::`-prefixed
value in the DB with an `--old` Fernet key, re-encrypts with `--new`, in one
psycopg2 transaction. The v2.31.1 `crypto_canary` row is both the pre-flight
validation gate and the post-flight verification.

Two changes:
1. **EDIT** `api/crypto.py` — add `encrypt_with_key()` / `decrypt_with_key()` /
   `fingerprint_of_key()` helpers so two keys can co-exist in one process.
2. **NEW** `scripts/rotate_encryption_key.py` — the CLI.

---

## Change 1 — api/crypto.py — append key-parameterised helpers

Append to the end of `api/crypto.py`. Do NOT modify any existing function.
...
## Change 2 — scripts/rotate_encryption_key.py — NEW FILE
...
```
git add -A
git commit -m "feat(security): v2.31.5 encryption key rotation CLI"
...
1. **Script loads**: `docker exec hp1_agent python -m scripts.rotate_encryption_key --help`
2. **Dry-run with correct old key**: Expect `Old key validated`, target count, `DRY-RUN complete`.
3. **Negative test — wrong old key**: Generate a random Fernet key, pass it as `--old`. Expect `Old key could not decrypt crypto_canary`, exit 3.
4. **Real rotation** (after stopping hp1_agent): Rotate, update `.env`, restart. `/api/status/health/crypto` should return new fingerprint.
5. **Old key now rejected**: Put old key back in `.env`, restart. Canary should return `status: mismatch`.
