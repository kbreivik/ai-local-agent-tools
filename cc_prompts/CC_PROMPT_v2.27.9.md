# CC PROMPT — v2.27.9 — FIELD_SCHEMA constants + TemplateCardRenderer component

## What this does
Defines the card field schema system and a generic renderer component:
1. New file `gui/src/schemas/cardSchemas.js` — CONTAINER_SCHEMA, SWARM_SERVICE_SCHEMA,
   CONTAINER_FIELD_RENDERERS, SWARM_FIELD_RENDERERS, and DEFAULT_TEMPLATES frontend mirror
2. New hook `gui/src/hooks/useCardTemplate.js` — resolves effective template for a card
   (per-connection override → type default → hardcoded default) with LRU cache
3. New component `gui/src/components/TemplateCardRenderer.jsx` — renders fields for a
   given phase (collapsed / expanded) using the schema + renderers + template
Version bump: 2.27.8 → 2.27.9

---

## Change 1 — gui/src/schemas/cardSchemas.js (NEW FILE)
...
## Change 2 — gui/src/hooks/useCardTemplate.js (NEW FILE)
...
## Change 3 — gui/src/components/TemplateCardRenderer.jsx (NEW FILE)
...
## Version bump
Update VERSION: 2.27.8 → 2.27.9

## Commit
```bash
git add -A
git commit -m "feat(schema): v2.27.9 FIELD_SCHEMA, TemplateCardRenderer, useCardTemplate hook"
