# CC PROMPT — v2.44.2 — feat(ui): TestsPanel complete overhaul — library, suites, compare, trend, schedule

## What this does

Replaces the existing `gui/src/components/TestsPanel.jsx` (291 lines, single-view)
with a full 5-tab test harness:

**Tab 1 — Library**: Filterable list of all TestCases. Multi-select → "Add to Suite"
or "Run selected". Filter by category, keyword search, critical/soft toggles.

**Tab 2 — Suites**: Named test groupings. Create/edit suite (pick tests + config:
memory on/off, memory backend). One-click run. Shows last score if run exists.

**Tab 3 — Results**: Run history list. Click run to expand full results.
Per-test pass/fail rows with steps, duration, tools called, failures.

**Tab 4 — Compare**: Select up to 4 run IDs from dropdowns. Side-by-side table:
each test row shows pass/fail per run. Delta column shows improvement/regression.
Export as HTML report button.

**Tab 5 — Trend & Schedule**: Score over time chart (sparkline per suite).
Cron schedule manager: add schedule (suite + cron expression + name), toggle
enable/disable, delete.

Version bump: 2.44.1 → 2.44.2.

---

## Change — replace `gui/src/components/TestsPanel.jsx` entirely
...
## Change 2 — `api/routers/tests_api.py` — extend RunTestsRequest

Add `test_ids` and `memory_enabled` fields to the existing request model:
...
In `_run_tests_bg`, before running tests, if `memory_enabled` is not None:
```python
...
CC: look at the existing `_run_tests_bg` signature and add these params through.

---

## Version bump

Update `VERSION`: `2.44.1` → `2.44.2`
...
```
git add -A
git commit -m "feat(ui): v2.44.2 TestsPanel complete overhaul — Library, Suites, Compare, Trend, Schedule"
