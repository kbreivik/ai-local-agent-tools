# CC PROMPT — v2.47.7 — feat(docs): auto-generated REFERENCE.md as living source-of-truth

## What this does
Builds a generator + Makefile target + CI integration that produces
`docs/REFERENCE.md` from live signals, so future debugging sessions can
grep one file instead of hunting through code each time.

The reference covers six sections, all extracted programmatically (no
hand-maintained drift):

1. **DB schema** — every user table from Postgres, columns, indexes, FKs
2. **Auth state per endpoint** — every `@router.<verb>` decorator in `api/routers/`,
   classified as auth-required or public based on `Depends(get_current_user)` presence
3. **WebSocket event types** — every distinct `manager.broadcast({"type": ...})`
   value the GUI consumes, with the source file/line
4. **Log channels** — short hand-maintained section explaining where each
   log type goes (stdout vs operation_log table vs WS vs Prometheus vs DB tables)
5. **Prometheus counter inventory** — every `Counter(...)` and `Gauge(...)` from
   `api/metrics.py` with name + labels + description
6. **Common diagnostic SQL queries** — pre-baked, named blocks for typical questions
   (latest test runs, failures by category, ops with no tool calls, harness fires,
   facts coverage, agent_attempts populated ratio, etc.)

The generator runs against the LIVE postgres instance for section 1 (so it
reflects whatever migrations have run, including module-managed tables not
in `api/db/models.py`), and against the source tree for sections 2/3/5
(AST-walked).

Sections 4 and 6 are hand-maintained markdown templates the generator
wraps into the output unchanged.

Outputs:
- New: `scripts/gen_reference.py` — the generator
- New: `docs/reference_templates/log_channels.md` — section 4 template
- New: `docs/reference_templates/diagnostic_queries.md` — section 6 template
- New: `docs/REFERENCE.md` — the generated artifact (committed; updated by CI)
- Updated: `Makefile` — add `make reference` target
- Updated: `.github/workflows/sensors.yml` — add reference-drift check job

Version bump: 2.47.6 → 2.47.7

---

## Why this exists

This session burned ~25 minutes on schema/auth/log-channel mismatches:
- `task` vs `label` column on `operations` table
- `/metrics` returning 401 (auth was added in v2.45.21, forgotten)
- `manager.send_line` writing to `operation_log` table + WS, not container stdout
- Prometheus counters defined but invisible until first `.inc()` call

Each was knowable from one or two grep commands but cost a round-trip to
discover. A checked-in reference doc that's *generated and CI-verified*
removes that cost. The generator is the source-of-truth; the doc is the
read-optimised view.

---

## Change 1 — `scripts/gen_reference.py` (NEW, ~400 lines)

Create this file. It has 6 phases that run in order and write a single
markdown file at the end.
    ...
    for t in schema["tables"]:
        ...
        if t["columns"]:
            ...
            for col in t["columns"]:
                ...
                lines.append(
                    f"| `{col['name']}` | {col_type} | "
                    f"{'✓' if col['nullable'] else '✗'} | {default} |"
                )
            lines.append("")
        if t["indexes"]:
...
def render_routes(routes: list[dict]) -> str:
    ...
    for r in routes:
        auth = "🔒 yes" if r["requires_auth"] else "🌐 public"
        lines.append(
...
CC: note `psycopg2` is an optional dependency. If not installed, the generator falls back to `docker exec` against `hp1-postgres` which is always present on agent-01. CI runs with `--no-db` to avoid needing PG in GitHub Actions; the schema section is then refreshed manually on agent-01 and committed. This is a known tradeoff for keeping the workflow simple.

---

## Change 2 — `docs/reference_templates/log_channels.md` (NEW)

Create this hand-maintained markdown:
...
## Change 3 — `docs/reference_templates/diagnostic_queries.md` (NEW)
...
## Change 4 — `Makefile` — add `reference` target
...
.PHONY: reference reference-check
...
CC: also add `reference reference-check` to the `.PHONY:` line if appropriate. Note `reference-check` uses `--no-db` because CI doesn't have PG reachable; only the non-DB sections are checked for drift. The DB schema section refreshes on `make reference` runs from agent-01.

---

## Change 5 — `.github/workflows/sensors.yml` — add reference-drift check

Add a new step to the existing `sensors` job. Find the existing
`Run sensor stack (full output)` step and add this BEFORE it:
...
CC: if the `Run sensor stack (full output)` step has `continue-on-error: true`,
preserve that pattern for the reference check too — but `false` is correct here
because reference drift IS a failure. Adjust based on existing surrounding patterns.

---

## Change 6 — Initial generation

After CC writes all files above:

1. Run `make reference` once to generate the initial `docs/REFERENCE.md`
2. Verify it's well-formed (open and skim sections 2, 3, 5 for accuracy — they
   come from AST walks and should match observable code)
3. The DB schema section will be missing in CC's environment; that's expected.
   It populates when run on agent-01.
...
# 2. Generate without DB (works anywhere)
python scripts/gen_reference.py --no-db
ls -la docs/REFERENCE.md
wc -l docs/REFERENCE.md

# 3. Re-run generation — output should be identical (modulo timestamp)
python scripts/gen_reference.py --no-db
git diff docs/REFERENCE.md
# Expected: only the "Generated: <ts>" line differs, OR nothing.

# 4. CI check should pass
make reference-check

# 5. Verify a known route gets correctly classified
grep -A1 '"path": .*/api/agent/run' docs/REFERENCE.md
# Expected: shows POST /api/agent/run with 🔒 yes (auth required)

grep -A1 '"path": .*/api/health' docs/REFERENCE.md  
# Expected: shows GET /api/health with 🌐 public (if /api/health exists)

# 6. Verify a known counter shows up
grep -i "harness_plan_nudges" docs/REFERENCE.md
# Expected: matches the v2.47.3 counter row in section 5
```

After deploying to agent-01, run `make reference` once on the host to populate
the schema section, commit the result.

---

## Version bump

Update `VERSION`: `2.47.6` → `2.47.7`
...
```
git add scripts/gen_reference.py \
        docs/reference_templates/ \
        docs/REFERENCE.md \
        Makefile \
        .github/workflows/sensors.yml \
        VERSION
git commit -m "feat(docs): v2.47.7 auto-generated REFERENCE.md as living source-of-truth"
git push origin main
```

After this lands:
- Future debugging starts with `cat docs/REFERENCE.md` instead of hunting through code
- Schema/auth/WS-event/counter answers are one grep away
- CI fails the build if anyone forgets to regenerate after a route or counter is added
- Operator regenerates the schema section on agent-01 after migrations

The investigation work continues in v2.47.8 (stronger clarification rejection +
no-evidence hint inside halluc guard), but with a working reference doc to
support it.
