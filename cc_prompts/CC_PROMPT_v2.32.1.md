# CC PROMPT — v2.32.1 — Structured system prompts: execute + build

## What this does
Restructures ACTION_PROMPT and BUILD_PROMPT in `api/agents/router.py` from flat prose
into explicitly labeled sections, same pattern as v2.32.0 applied to observe + investigate.
This is a **representation-only** change — every rule, tool chain, and edge case is preserved.
No runtime code changes. No logic changes.

Version bump: 2.32.0 → 2.32.1

## Change 1 — api/agents/router.py — Replace ACTION_PROMPT

Find the existing `ACTION_PROMPT = """..."""` string and replace it entirely with:
...
## Change 2 — api/agents/router.py — Replace BUILD_PROMPT

Find the existing `BUILD_PROMPT = """..."""` string and replace it entirely with:
...
Confirm these alias lines exist unchanged after the replacements:
...
Update VERSION file: 2.32.0 → 2.32.1

## Commit

```bash
git add -A
git commit -m "refactor(agents): v2.32.1 structured system prompts for execute + build

Restructure ACTION_PROMPT and BUILD_PROMPT from flat prose into
explicitly labeled sections (Role, Environment, Constraints, Tool Budget,
Tool Chains, Completion Conditions, Response Style).

Completes the Harness Tightening prompt restructuring started in v2.32.0.
All 4 agent system prompts now use consistent ═══ SECTION ═══ separators.

No logic changes — every rule, tool chain, and edge case preserved.
Representation-only change for improved model parsing reliability."
