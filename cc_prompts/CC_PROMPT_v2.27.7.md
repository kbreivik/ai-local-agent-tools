# CC PROMPT — v2.27.7 — DB: card_templates + display_aliases tables + API

## What this does
Two new DB tables + API routes for the card template and entity alias systems:
1. `card_templates`: stores ordered field arrays per section (header_sub, collapsed, expanded,
   entity_only, hidden) scoped by card type ('container', 'swarm_service') or per connection UUID
2. `display_aliases`: stores editable display name overrides for entities (container name →
   user-set alias; falls back to `origin` when alias cleared)

Both tables get full CRUD API routes and are seeded with hardcoded defaults on init.
Version bump: 2.27.6 → 2.27.7

---

## Change 1 — api/db/card_templates.py (NEW FILE)
...
## Change 2 — api/db/display_aliases.py (NEW FILE)
...
## Change 3 — api/routers/card_templates.py (NEW FILE)
...
async def get_connection_template(connection_id: str, _: str = Depends(get_current_user)):
    ...
    # card_type hint from query param
    from fastapi import Query
    from api.db.card_templates import resolve_template, get_template
    override = get_template('connection', connection_id)
    return {
...
## Change 4 — api/routers/display_aliases.py (NEW FILE)
...
## Change 5 — api/main.py: mount new routers + init new tables

NOTE for CC: Read api/main.py first to find the exact router mounting pattern and the startup
init section (where init_connections(), init_credential_profiles(), etc. are called).
Then add the following:

**Import and mount both new routers** in the same location as other routers:
```python
...
**Init both new tables** in the startup section:
```python
...
## Version bump
Update VERSION: 2.27.6 → 2.27.7

## Commit
```bash
git add -A
git commit -m "feat(db): v2.27.7 card_templates + display_aliases tables and API routes"
