# CC PROMPT — v2.41.2 — refactor(agents): extract step_guard.py — hallucination + fabrication

## Context

SPEC: `cc_prompts/SPEC_v2.41_AGENT_SPLIT.md`
Depends on: v2.41.1 (step_llm) must be DONE first.

Extracts the hallucination guard, fabrication detector, and their shared
external-AI gate_failure routing into `api/agents/step_guard.py`.

The guard runs on the `finish=="stop" and not msg.tool_calls` path.
Before it, there is a plan_action safety check (execute/action agents).
Both move together into step_guard.py.

Pure refactor — zero logic change.

Version bump: 2.41.1 → 2.41.2.

---

## Change 1 — create `api/agents/step_guard.py`
...
## Change 2 — `api/routers/agent.py` — wire step_guard
...
Inside the while loop, locate the `if finish == "stop" and not msg.tool_calls:` block.
Replace everything from the plan_action safety check through the fabrication
exhausted `break` with:
...
The `# GuardOutcome.PROCEED` path continues to the existing degraded-findings
synthesis and done broadcast that follow this block. Those stay in agent.py for now.
...
```bash
python -c "from api.agents.step_guard import run_stop_path_guards, GuardOutcome; print('ok')"
...
```
git add -A
git commit -m "refactor(agents): v2.41.2 extract step_guard.py — hallucination + fabrication guards"
