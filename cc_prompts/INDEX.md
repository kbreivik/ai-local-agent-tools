...
| CC_PROMPT_v2.36.7.md | v2.36.7 | fix(logs): populate operations.model_used — insert seed + trace backfill | RUNNING |
...
Logs → Operations view has been showing a blank Model column for every agent-started run. Kent spotted it on 2026-04-20 looking at two consecutive v2.36.5 verification runs (20:55:35 rendered blank, 20:58:21 rendered "—" — two UI renderings of an empty/null DB value). The live WebSocket feed correctly broadcasts `Model: qwen/qwen3.6-35b-a3b` on every step, but `operations.model_used` stays empty at the DB level. Root cause in two layers: (1) `api/routers/agent.py::run_agent` and `run_subtask` both use `logger_mod.log_operation(session_id, task, owner_user=user)` — the 3-arg legacy alias in `api/logger.py` silently drops `model_used` when forwarding to `log_operation_start`, so every agent-started operation inserts with `model_used=''`; (2) `log_operation_complete` in the terminal write path only updates status + duration, never `model_used`, so the empty insert value persists. Meanwhile v2.36.0's `_extract_response_model` correctly populates `agent_llm_traces.model` with the REAL API-reported model for every LLM step — including the step_index=99999 sentinel row v2.36.3 writes for external-AI escalations carrying `claude-sonnet-4-6` etc. The Operations view data source was never bridged to this trace column. Fix adds both halves: `log_operation` alias gains a `model_used` kwarg that forwards to `log_operation_start` (backward compatible with every existing caller's 3-arg call); `run_agent` and `run_subtask` pass `model_used=_lm_model()` at insert so every op has a seed from insert onwards; `log_operation_complete` gains a best-effort COALESCE subquery that backfills `operations.model_used` from the highest-step_index `agent_llm_traces` row on terminal write (picks up v2.36.3's step_index=99999 for external-AI runs and the last local step for everything else, filtering `model IS NOT NULL AND model <> ''` so pre-v2.36.0 empty-model rows don't nuke the seed). Backfill runs in the same transaction as `complete_operation`, wrapped in try/except at debug level so a schema mismatch can't block the terminal status write. 4 regression tests in `tests/test_operations_model_column.py` cover the alias kwarg forwarding (locks in today's regression), the external-AI step_index=99999 pickup (locks in that the backfill respects v2.36.0's provenance work), the no-trace seed-preservation path, and the empty-model-filter path. No schema changes — `operations.model_used` and `agent_llm_traces.model` both pre-exist.
