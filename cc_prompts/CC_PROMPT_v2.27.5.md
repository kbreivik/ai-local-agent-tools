# CC PROMPT — v2.27.5 — Frontend: Discovered view (nav + DiscoveredView component)

## What this does
New nav item "Discovered" under MONITOR section in Sidebar.jsx.
New component `gui/src/components/DiscoveredView.jsx`:
- Harvest button → POST /api/discovery/harvest → populates table
- Table: source badge, IP/hostname, platform guess, status, linked/unlinked indicator
- Per-row "Test with profile" — profile dropdown + Test button → inline result
- Per-row "Create connection" — appears after successful test, opens pre-filled connection form
- Filter: source (all/proxmox/unifi/swarm/manual), status (all/unlinked)
- Compare vs existing connections (linked indicator from API)
Route: 'Discovered' in App.jsx nav routing.
Version bump: 2.27.4 → 2.27.5

---

## Change 1 — gui/src/components/Sidebar.jsx: add Discovered nav item
...
## Change 2 — gui/src/components/DiscoveredView.jsx (NEW FILE)
...
## Change 3 — gui/src/App.jsx: add Discovered routing

NOTE for CC: Read App.jsx to find the exact routing pattern used for tabs like 'Cluster', 'Logs', 'Memory', etc. Then add 'Discovered' in the same pattern, importing DiscoveredView from './components/DiscoveredView'.

The import to add at the top of App.jsx:
```jsx
import DiscoveredView from './components/DiscoveredView'
```

The routing case to add (in the same pattern as other MONITOR tabs):
```jsx
{activeTab === 'Discovered' && <DiscoveredView />}
```

---

## Version bump
Update VERSION: 2.27.4 → 2.27.5

## Commit
```bash
git add -A
git commit -m "feat(ui): v2.27.5 Discovered view — passive harvest, test-with-profile, one-click link"
