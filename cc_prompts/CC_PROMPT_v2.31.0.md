# CC PROMPT — v2.31.0 — feat(docs): Bookstack sync — periodic harvest into RAG doc_chunks

## What this does
New subsystem: periodic sync from Bookstack API into the `doc_chunks` pgvector table so
that the existing doc search (Browse + Ask modes) indexes live Bookstack content.
Four changes across four files:
1. New `api/rag/bookstack_sync.py` — Bookstack API harvester
2. `api/routers/docs.py` — manual trigger + status endpoints
3. `api/main.py` — background scheduler wired to lifespan
4. `api/routers/settings.py` — two new settings keys seeded on startup
Version bump: v2.30.1 → v2.31.0

---

## Change 1 — api/rag/bookstack_sync.py — NEW FILE
...
## Change 2 — api/routers/docs.py — add Bookstack sync endpoints

Append these two routes to the end of `api/routers/docs.py` (before any final blank
lines). Do not modify any existing routes.
...
## Change 3 — api/main.py — wire Bookstack scheduler to lifespan

In `api/main.py`, find the lifespan context manager (the `@asynccontextmanager` that
starts collectors, notifications, etc.). Add Bookstack scheduler start/stop alongside
the existing startup/shutdown calls.

Find the section inside the lifespan that starts background services — it will look
something like:
...
```python
    ...
    try:
        ...
        log.warning("Bookstack scheduler start failed: %s", _bs_err)
```

And in the shutdown section (after `yield`, before the lifespan context closes), add:
...
## Change 4 — api/routers/settings.py — seed two new settings keys

In the `seed_default_settings()` function (or equivalent startup seeding function),
add these two keys with their defaults. Find the block where other settings are seeded
(e.g., `autoUpdate`, `ghcrToken`, `agentHostIp`) and add:

```python
    ("bookstackSyncEnabled",       False),
    ("bookstackSyncIntervalHours", 6),
```

Use the same pattern already used for other boolean/numeric settings in that file.

---

## Version bump
Update `VERSION` in `api/constants.py`: `v2.30.1` → `v2.31.0`

## Commit
```
git add -A
git commit -m "feat(docs): v2.31.0 Bookstack sync — periodic harvest into RAG doc_chunks"
